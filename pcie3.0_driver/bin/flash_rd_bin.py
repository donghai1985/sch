...
def flash_reset():
    wr_reg(0x000000DC,0x00000001)
    wr_reg(0x000000DC,0x00000000)
    wr_reg(0x000000FC,0x00000000)
    wr_reg(0x000800A0,0x00000001)
    wr_reg(0x000800A0,0x00000000)

def flash_rd_bin():
    flash_reset()
    cfg_init()
    time.sleep(1)
    wr_reg(0x000000F4,0x01000000) # rd_len
    # wr_reg(0x000000F4,0x00002000) # rd_len
    wr_reg(0x000000F8,0x00000000) # base_addr
    wr_reg(0x000000F0,0x00000000)
    wr_reg(0x000000F0,0x00000001) # trig
    wr_reg(0x000000F0,0x00000000)

def flash_rd_file():
    cmd = 'xdma_rw.exe c2h_1 read 0x00000000 -b -f data8m_0.bin -l 8388608'
    os.popen(cmd).read()
    cmd = 'xdma_rw.exe c2h_1 read 0x00000000 -b -f data8m_1.bin -l 8388608'
    os.popen(cmd).read()
